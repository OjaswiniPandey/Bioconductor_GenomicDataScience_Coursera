---
title: "Bioconductor_GenomicDataScience_Quiz2"
author: "Ojaswini Pandey"
date: "2023-04-22"
output: html_document
---

Q1 : What is the GC content of “chr22” in the “hg19” build of the human genome? Tip: The reference genome includes “N” bases; you will need to exclude those. 

```{r}
library(AnnotationHub)
ahub = AnnotationHub()
ahub
```


```{r}
library(Biostrings)
library(BSgenome)
library(rtracklayer)

BiocManager::install("BSgenome.Hsapiens.UCSC.hg19")

library("BSgenome.Hsapiens.UCSC.hg19")


#counting total bases on chromosome 22
alphabet_frequency = alphabetFrequency(Hsapiens$chr22)
total_bases = sum(alphabet_frequency[c('A','G','C','T')])

#counting GC bases on chrmosome 22
GC_bases = sum(alphabet_frequency[c('G','C')])

#calculating GC ratio
GC_content = GC_bases/total_bases
GC_content

```
Q2 : Background: In the previous assessment we studied H3K27me3 “narrowPeak” regions from the H1 cell line (recall that the Roadmap ID for this cell line is “E003”). We want to examine whether the GC content of the regions influence the signal; in other words wether the reported results appear biased by GC content.
What is mean GC content of H3K27me3 “narrowPeak” regions from Epigenomics Roadmap from the H1 stem cell line on chr 22.
Clarification: Compute the GC content for each peak region as a percentage and then average those percentages to compute a number between 0 and 1.

```{r}

# retrieving the record
H3K27me3_qh = query(ahub, c("H3K27me3", "E003", "narrowPeak"))
H3K27me3_qh
H3K27me3_record = H3K27me3_qh[["AH29892"]]

# extracting chromosome 22 and sequences
H3K27me3_chr22 = subset(H3K27me3_record, seqnames == "chr22")

H3K27me3_chr22_views = Views(Hsapiens, H3K27me3_chr22)

# calculating the mean GC content
GC_contents = letterFrequency(H3K27me3_chr22_views, "GC", as.prob = TRUE)

mean_GC = mean(GC_contents)
mean_GC



```
Q3 : The “narrowPeak” regions includes information on a value they call “signalValue”. What is the correlation between GC content and “signalValue” of these regions (on chr22)?

```{r}

signal_value = mcols(H3K27me3_chr22_views)$signalValue
cor(signal_value, GC_contents)


```

Q4 : The “narrowPeak” regions are presumably reflective of a ChIP signal in these regions. To confirm this, we want to obtain the “fc.signal” data from AnnotationHub package on the same cell line and histone modification. This data represents a vector of fold-change enrichment of ChIP signal over input.
What is the correlation between the “signalValue” of the “narrowPeak” regions and the average “fc.signal” across the same regions?
Clarification: First compute the average “fc.signal” for across each region, for example using “Views”; this yields a single number of each region. Next correlate these numbers with the “signalValue” of the “narrowPeaks”.


```{r}

# retrieving the record
H3K27me3_fc = query(ahub, c("H3K27me3", "E003", "fc.signal"))
H3K27me3_fc
H3K27me3_fc_record <- H3K27me3_fc[["AH32033"]]



# getting subset data on chromosome 22
library(GenomicRanges)
gr22 = GRanges(seqnames = "chr22", ranges = IRanges(start = start(Hsapiens$chr22), end = end(Hsapiens$chr22)))
H3K27me3_fc_gr = import(H3K27me3_fc_record, which = gr22, as = "Rle")
H3K27me3_fc_gr22 = H3K27me3_fc_gr$chr22


# viewing fc.signal data
fc.signal = Views(H3K27me3_fc_gr22, start = start(H3K27me3_chr22), end = end(H3K27me3_chr22))

# calculating the correlation between the average of fc.signal and signalValue
fc.signal_mean = mean(fc.signal)
cor(fc.signal_mean, signal_value)


```

Q5 : Referring to the objects made and defined in the previous question. How many bases on chr22 have an fc.signal greater than or equal to 1?

```{r}

sum(H3K27me3_fc_gr22 >= 1)

```
Q6 : The H1 stem cell line is an embryonic stem cell line, a so-called pluripotent cell. Many epigenetic marks change upon differentiation. We will examine this. We choose the cell type with Roadmap ID “E055” which is foreskin fibroblast primary cells. We will use the “fc.signal” for this cell type for the H3K27me3 mark, on chr22. We now have a signal track for E003 and a signal track for E055. We want to identify regions of the genome which gain H3K27me3 upon differentiation. These are regions which have a higher signal in E055 than in E003. To do this properly, we would need to standardize (normalize) the signal across the two samples; we will ignore this for now.
Identify the regions of the genome where the signal in E003 is 0.5 or lower and the signal in E055 is 2 or higher.
Tip: If you end up with having to intersect two different Views, note that you will need to convert the Views to IRanges or GRanges first with ir<-as(vi,“IRanges”).

```{r}

#getting the record
H3K27me3_E055 = query(ahub, c("H3K27me3", "E055"))
H3K27me3_E055
H3K27me3_E055_record = H3K27me3_E055[["AH32470"]]

# getting subset data on chromosome 22
gr_chr22 = GRanges(seqnames = "chr22", ranges = IRanges(start = start(Hsapiens$chr22), end = end(Hsapiens$chr22)))
H3K27me3_fc_gr_E055 = import(H3K27me3_E055_record, which = gr_chr22, as = "Rle")
H3K27me3_fc_gr22_E055 = H3K27me3_fc_gr_E055$chr22

# finding regions of the genome where the signal in E003 is 0.5 or lower and the signal in E055 is 2 or higher.
region_E003 = as(slice(H3K27me3_fc_gr22, upper = 0.5), "IRanges")
region_E055 = as(slice(H3K27me3_fc_gr22_E055, lower = 2), "IRanges")
inter_region = intersect(region_E003, region_E055)
sum(width(inter_region))


```

Q7 : CpG Islands are dense clusters of CpGs. The classic definition of a CpG Island compares the observed to the expected frequencies of CpG dinucleotides as well as the GC content. Specifically, the observed CpG frequency is just the number of “CG” dinucleotides in a region. The expected CpG frequency is defined as the frequency of C multiplied by the frequency of G divided by the length of the region.
What is the average observed-to-expected ratio of CpG dinucleotides for CpG Islands on chromosome 22?

```{r}

# retrieving the cpg
ahub_human = subset(ahub, species == "Homo sapiens")
ahub_human_cpg = query(ahub_human, "CpG Islands")
ahub_human_cpg
ahub_human_cpg_record = ahub_human_cpg[["AH5086"]]

# getting subset data on chromosome 22
ahub_human_cpg_chr22 = subset(ahub_human_cpg_record, seqnames == "chr22")
ahub_human_cpg_chr22_views = Views(Hsapiens, ahub_human_cpg_chr22)

# calculating observed GC bases
observed_GC = dinucleotideFrequency(ahub_human_cpg_chr22_views)[,7]/width(ahub_human_cpg_chr22_views)

# calculating expected GC bases
freq_C = letterFrequency(ahub_human_cpg_chr22_views, "C")
freq_G = letterFrequency(ahub_human_cpg_chr22_views, "G")
expected_GC = (freq_C/width(ahub_human_cpg_chr22_views))*(freq_G/width(ahub_human_cpg_chr22_views))

# calculating the average observed-to-expected ratio of CpG dinucleotides
mean(observed_GC/expected_GC)


```

Q8 : A TATA box is a DNA element of the form “TATAAA”. Around 25% of genes should have a TATA box in their promoter. We will examine this statement. How many TATA boxes are there on chr 22 of build hg19 of the human genome?

```{r}

TATA_boxes = countPattern("TATAAA", Hsapiens$chr22) + countPattern("TATAAA", reverseComplement(Hsapiens$chr22))
TATA_boxes

```

Q9

Q10 : It is possible for two promoters from different transcripts to overlap, in which case the regulatory features inside the overlap might affect both transcripts. This happens frequently in bacteria. How many bases on chr22 are part of more than one promoter of a coding sequence?
Clarification: Use the TxDb.Hsapiens.UCSC.hg19.knownGene package to define transcripts and coding sequence. Here, we define a promoter to be 900bp upstream and 100bp downstream of the transcription start site. In this case, ignore strand in the analysis.


```{r}
# calculate transcript lengths
trans_len_chr22 <- transcriptLengths(txdb, with.cds_len = TRUE)
trans_len_chr22 <- trans_len_chr22[trans_len_chr22$cds_len > 0,]

# find promoters from different transcripts to overlap
trans_eval <- proms[mcols(proms)$tx_id %in% trans_len_chr22$tx_id]
result = sum(coverage(trans_eval) > 1)
result["chr22"]


```






